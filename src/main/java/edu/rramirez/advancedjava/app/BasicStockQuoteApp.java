...
public class BasicStockQuoteApp {
	...
	private enum ProgramTerminationStatusEnum {
		 ...
		 *
		 * @param statusCodeValue the value to return the OS. A value of 0 indicates
		 *                        success or normal termination. non 0 numbers indicate
	 ...
	 *                     stock queries.
	 *                     <p/>
	 *                     NOTE: this is a example of Dependency Injection in
	 *                     action.
	 */
	public BasicStockQuoteApp(StockService stockService) {
	...
	/**
	 * Given a <CODE>stockQuery</CODE> get back a the info about the stock to
	 * display to the user.
	 *
	 * @param stockQuery the stock to get data for.
	 * @return a String with the stock data in it.
	 * @throws StockServiceException If data about the stock can't be retrieved.
	...
	public String displayStockQuotes(StockQuery stockQuery) throws StockServiceException {
		...
		List<StockQuote> stockQuotes = stockService.getQuote(stockQuery.getSymbol(), stockQuery.getFrom(),
				stockQuery.getUntil());
	 ...
	 * 
	 * 
	 * @param statusCode        an enum value that indicates if the program
	 *                          terminated ok or not.
	 * 
	 * @param diagnosticMessage A message to display to the user when the program
	 *                          ends. This should be an error message in the case of
	 *                          abnormal termination
	 *                          <p/>
	 *                          NOTE: This is an example of DRY in action.
	 *                          A program should only have one exit point. This
	 *                          makes it easy to do any clean up operations before
	 *                          a program quits from just one place in the code.
	...
	public static void main(String[] args) {
		...
		try {
			...
			BasicStockQuoteApp basicStockQuoteApp = new BasicStockQuoteApp(stockService);
